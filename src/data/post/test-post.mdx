---
publishDate: 2026-06-11T00:00:00Z
title: Test post
excerpt: >-
  While easy to get started, Astrowind is quite complex internally.  This page
  provides documentation on some of the more intricate parts.
image: >-
  https://images.unsplash.com/photo-1534307671554-9a6d81f4d629?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1651&q=80
category: Documentation
tags:
  - astro
  - tailwind css
  - front-end
metadata:
  canonical:
---
A new test post with some words.

* A list
* Another list item
  * Do we have styling for indented lists?
* A list continued

> A quote - Test Richardson